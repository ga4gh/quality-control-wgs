...
In the [NPM-sample-QC](References.md#npm-sample-qc) reference implementation it is computed using [samtools stats](References.md#samtools-stats), reporting the total number of [high quality reads](terminologies.md#high-quality-reads) from short paired-end sequencing, continuous long-read sequencing, synthetic long-read sequencing (read level), and [samtools stats](References.md#samtools-stats) flag-xxxxx for synthetic long-read sequencing (template level), excluding supplementary and secondary reads. Duplicated reads are included and no mapping qualiy is applied.
